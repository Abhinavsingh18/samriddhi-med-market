    ...
    setCartItems(prevItems => {
      ...
      if (existingItem) {
        ...
        if (quantity > product.stock) {
          toast.error("Cannot add this many items - stock limit reached");
  ...
  const updateQuantity = (productId: string, quantity: number) => {
    setCartItems(prevItems => {
      ...
      if (productToUpdate && quantity <= 0) {
        toast.info(`Removed ${productToUpdate.product.name} from your cart`);
        return prevItems.filter(item => item.product.id !== productId);
      }
      
      if (productToUpdate && quantity > productToUpdate.product.stock) {
        toast.error("Cannot add more of this item - stock limit reached");
        return prevItems.map(item => 
          item.product.id === productId 
            ? { ...item, quantity: productToUpdate.product.stock } 
            : item
        );
      }
